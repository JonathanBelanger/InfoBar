Public Class InfoBarModuleMain
  ...
  Private Const InfoBarModuleGUID As String = "{9A63CDD4-4139-4953-81DD-AB0209CA2181}"

#Region "Windows API"
  Private Enum AC_LINE_STATUS As Byte
    Offline = 0
    Online = 1
    Unknown = 255
  End Enum

  Private Enum BATTERY_FLAG As Byte
    High = 1
    Low = 2
    Critical = 4
    Charging = 8
  ...
  Private Structure SYSTEM_POWER_STATUS
    Public ACLineStatus As AC_LINE_STATUS
    Public BatteryFlag As BATTERY_FLAG
  ...
  Private Declare Function GetSystemPowerStatus Lib "kernel32.dll" (ByRef lpSystemPowerStatus As SYSTEM_POWER_STATUS) As Integer
#End Region
...
#Region "Module Information"
  ...
  Public Overrides ReadOnly Property ModuleDescription() As String
    Get
      Return "Displays the amount of battery remaining, as well as other useful battery information."
    End Get
  End Property
...
#Region "Module Initialization/Finalization/Updates"
  ...
  Public Overrides Sub TimerTick(ByRef bModuleIsDirty As Boolean)
    ...
    ' Do any work here
    Dim stats As New SYSTEM_POWER_STATUS
    GetSystemPowerStatus(stats)

    If stats.BatteryFlag = BATTERY_FLAG.NoSystemBattery Then
      m_Text = vbNullString
      m_TooltipText = "No Battery Installed"
    Else

      Dim sPct As String = stats.BatteryLifePercent & "%"
      m_Text = sPct

      Select Case stats.ACLineStatus
        Case AC_LINE_STATUS.Online
          m_TooltipText = "Battery Life: " & sPct & " (Charging)" & vbCrLf
        Case AC_LINE_STATUS.Offline
          m_TooltipText = "Battery Life: " & sPct & vbCrLf
        Case AC_LINE_STATUS.Unknown
          m_TooltipText = "Battery Life: " & sPct & " (Unknown)" & vbCrLf
      End Select

      If stats.BatteryLifeTime > -1 Then
        m_TooltipText &= "Time Remaining: " & Utilities.FormatTime(stats.BatteryLifeTime) & vbCrLf
      Else
        m_TooltipText &= "Time Remaining: (Unknown)" & vbCrLf
      End If

      If stats.BatteryFullLifeTime > -1 Then
        m_TooltipText &= "Current Maximum Lifetime: " & Utilities.FormatTime(stats.BatteryFullLifeTime) & vbCrLf
      Else
        m_TooltipText &= "Current Maximum Lifetime: (Unknown)" & vbCrLf
      End If

      m_TooltipText &= "Charge Status: " & stats.BatteryFlag.ToString & vbCrLf
    End If
...
#Region "Mouse/Keyboard/Menu Processing"
  ...
  Public Overrides Sub ProcessMouseUp(ByVal e As System.Windows.Forms.MouseEventArgs, ByRef bWindowIsDirty As Boolean)
    ...
    If m_ButtonBounds.Contains(pt) Then
      If m_ButtonState <> 1 Then
        ...
        ShellExecute(IntPtr.Zero, "open", "RunDll32.exe", "Shell32.dll,Control_RunDLL powercfg.cpl", vbNullString, 1)
      End If
    Else
      If m_ButtonState <> 0 Then
        m_ButtonState = 0
  ...
  Public Overrides Sub AddMainPopupMenuItems()
    ...
    MainMenu.AddMenuItem(InfoBarModuleGUID & "::" & "SETTINGS", "Battery Status Settings...", My.Resources.icon, True, True)
  ...
  Public Overrides Sub ProcessMenuItemClick(ByVal Key As String)
    Select Case Key
      Case InfoBarModuleGUID & "::" & "SETTINGS"    ' Show Battery Status Settings...
