Friend Class Settings
  ...
  Public Caller As EVERESTSensorMonitor.InfoBarModuleMain
  ...
  Private Sub chkShowIcon_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkShowIcon.CheckedChanged
    If chkShowIcon.Checked = False And chkShowText.Checked = False Then
      ...
      MsgBox("At least one display option must be selected.", MsgBoxStyle.Information Or MsgBoxStyle.OkOnly, "EVEREST Sensor Monitor")
    Else
      IBSettings.EnableSettingsDialogApplyButton()
    End If
  End Sub
  ...
  Public Sub ResetEverestData()
    ...
    Caller.Everest_ResetData()

    If Caller.Everest_GetData = True Then

      Dim LI As ListViewItem
      For Each entry As EVERESTSensorMonitor.InfoBarModuleMain.EverestDataEntry In Caller.EverestData.Temperatures
        LI = New ListViewItem
        LI.Name = entry.ID
        LI.Text = entry.Label
      ...
      For Each entry As EVERESTSensorMonitor.InfoBarModuleMain.EverestDataEntry In Caller.EverestData.FanSpeeds
        LI = New ListViewItem
        LI.Name = entry.ID
        LI.Text = entry.Label
      ...
      For Each entry As EVERESTSensorMonitor.InfoBarModuleMain.EverestDataEntry In Caller.EverestData.Voltages
        LI = New ListViewItem
        LI.Name = entry.ID
        LI.Text = entry.Label
